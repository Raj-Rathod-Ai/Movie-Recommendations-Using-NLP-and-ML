...
def get_movie_insights(movie_title: str, overview: str = "", genres: str = "", access_key: str = None) -> dict:
    ...
    try:
        ...
        return {
            ...
            "fun_fact": "This film stands out in its genre for its distinct direction."
        }

def get_smart_fallback_recommendations(query: str, top_n: int = 10, access_key: str = None) -> list:
    """
    Generate movie recommendations when local matrix finds no match.
    """
    client = get_client(access_key)
    if not client:
        return [
            ...
            },
            {
                ...
                "vote_average": 8.6,
                "popularity": 42.1,
                "overview": "When Earth becomes uninhabitable in the future, a farmer and ex-NASA pilot is tasked to pilot a spacecraft to find a new planet for humans."
            },
            {
                ...
                "vote_average": 9.0,
                "popularity": 50.0,
                "overview": "When the menace known as the Joker wreaks havoc and chaos on Gotham, Batman must accept one of the greatest psychological and physical tests of his ability."
            }
        ][:top_n]

    prompt = f"""
    The user is looking for movie recommendations related to: "{query}".
    ...
    - "genres": Main genres (e.g. "Sci-Fi Action")
    - "vote_average": Estimated rating out of 10 (e.g. 8.5)
    ...
    try:
        ...
        logger.warning(f"Fallback recommendation error: {e}")
        return []
